...
처음부터 entities를 세분화하지 말 것. 요구사항이 안정화될 때까지 해당 `page`나 `feature` 슬라이스 안(도메인 모델을 둔 세그먼트 — 흔히 `model/`)에 코드를 두고, 나중에 리팩토링으로 entities로 올려라.
...
아래는 `ui/model/api` 패턴을 따를 때의 예시다 — 세그먼트 구성은 자유 ([[segment-rules]]).
