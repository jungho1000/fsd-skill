...
├── entities/  ← 단일 도메인 엔티티 (User, Product, Order...)
...
├─────────────┤
│   pages     │  보통 한 화면 = 한 슬라이스. 비슷한 페이지는 한 슬라이스로 묶일 수 있음
├─────────────┤
...
**pages** — "화면 단위 슬라이스"  
보통 한 페이지가 한 슬라이스지만, 비슷한 페이지(로그인/회원가입, 상품 목록/상세 등)는 *하나의 슬라이스*로 묶일 수 있다.  
...
**Step 5: 두 곳 이상에서 반복되는 단일 도메인 엔티티이고, DTO 변환·파생 데이터·도메인 액션 중 하나라도 있는가?**
- 둘 다 충족 → `entities/`
- 반복은 있으나 변환 없음 → `shared/api/`에서 re-export로 이름만 부여
